...
CREATE POLICY "allow_all_profile_roles"
...
BEGIN
  ...
  -- Add ExCom role if not already assigned
  IF user_id IS NOT NULL AND excom_role_id IS NOT NULL THEN
    INSERT INTO profile_roles (profile_id, role_id)
    VALUES (user_id, excom_role_id)
    ON CONFLICT (profile_id, role_id) DO NOTHING;
  END IF;
END $$;
