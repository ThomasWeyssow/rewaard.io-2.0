...
BEGIN
  ...
  IF NEW.next_nomination_start_date IS NOT NULL THEN
    ...
    NEW.next_nomination_start_date := date_trunc('day', NEW.next_nomination_start_date);
    
    IF NEW.next_nomination_period = 'monthly' THEN
      -- End date is last day of the month at 23:59:59
      NEW.next_nomination_end_date := (date_trunc('month', NEW.next_nomination_start_date) + interval '1 month' - interval '1 second')::date;
    ELSE -- 'bi-monthly'
      -- End date is last day of next month at 23:59:59
      NEW.next_nomination_end_date := (date_trunc('month', NEW.next_nomination_start_date) + interval '2 month' - interval '1 second')::date;
    END IF;
  END IF;
  ...
  IF TG_OP = 'UPDATE' AND 
    ...
    NEW.next_nomination_start_date := date_trunc('day', NEW.next_nomination_start_date); -- Ensure midnight
