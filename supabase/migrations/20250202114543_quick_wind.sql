...
BEGIN
  ...
  IF NEW.next_nomination_start_date IS NOT NULL THEN
    ...
    IF NEW.next_nomination_period = 'monthly' THEN
      ...
      NEW.next_nomination_end_date := (date_trunc('month', NEW.next_nomination_start_date) + interval '1 month')::date - interval '1 second';
    ELSE -- 'bi-monthly'
      -- End date is last day of next month at 23:59:59
      NEW.next_nomination_end_date := (date_trunc('month', NEW.next_nomination_start_date) + interval '2 month')::date - interval '1 second';
    END IF;
  END IF;
  ...
  IF TG_OP = 'UPDATE' AND 
    ...
    -- Reset next cycle
    NEW.next_nomination_start_date := date_trunc('day', NEW.ongoing_nomination_end_date + interval '1 day');
