...
BEGIN
  ...
  IF NEW.next_nomination_start_date IS NOT NULL AND NEW.next_nomination_end_date IS NULL THEN
    ...
    IF NEW.next_nomination_period = 'monthly' THEN
      -- Add exactly one month minus 1 second
      NEW.next_nomination_end_date := NEW.next_nomination_start_date + interval '1 month' - interval '1 second';
    ELSE -- 'bi-monthly'
      -- Add exactly two months minus 1 second
      NEW.next_nomination_end_date := NEW.next_nomination_start_date + interval '2 months' - interval '1 second';
    END IF;
  END IF;
