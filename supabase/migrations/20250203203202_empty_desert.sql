...
RETURNS TRIGGER AS $$
DECLARE
  paris_start timestamptz;
  paris_end timestamptz;
BEGIN
  ...
  IF NEW.next_nomination_start_date IS NOT NULL AND NEW.next_nomination_end_date IS NULL THEN
    ...
    paris_start := NEW.next_nomination_start_date AT TIME ZONE 'UTC' AT TIME ZONE 'Europe/Paris';
    paris_start := date_trunc('day', paris_start);
    NEW.next_nomination_start_date := (paris_start AT TIME ZONE 'Europe/Paris' AT TIME ZONE 'UTC')::timestamptz;
    
    IF NEW.next_nomination_period = 'monthly' THEN
      -- Add exactly one month and set to 23:59:59 Paris time
      paris_end := date_trunc('month', paris_start) + interval '1 month' - interval '1 second';
      NEW.next_nomination_end_date := (paris_end AT TIME ZONE 'Europe/Paris' AT TIME ZONE 'UTC')::timestamptz;
    ELSE -- 'bi-monthly'
      -- Add exactly two months and set to 23:59:59 Paris time
      paris_end := date_trunc('month', paris_start) + interval '2 month' - interval '1 second';
      NEW.next_nomination_end_date := (paris_end AT TIME ZONE 'Europe/Paris' AT TIME ZONE 'UTC')::timestamptz;
    END IF;
  END IF;
  ...
  IF TG_OP = 'UPDATE' AND 
    ...
    paris_end := NEW.ongoing_nomination_end_date AT TIME ZONE 'UTC' AT TIME ZONE 'Europe/Paris';
    paris_start := date_trunc('day', paris_end + interval '1 day');
    NEW.next_nomination_start_date := (paris_start AT TIME ZONE 'Europe/Paris' AT TIME ZONE 'UTC')::timestamptz;
