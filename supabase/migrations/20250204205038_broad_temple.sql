-- Update check_and_update_nomination_cycles function with better error handling
...
BEGIN
  ...
  IF settings_record.ongoing_cycle_id IS NOT NULL THEN
    SELECT * INTO ongoing_cycle
    FROM nomination_cycles
    WHERE id = settings_record.ongoing_cycle_id;

    IF ongoing_cycle IS NULL THEN
      -- Fix invalid ongoing cycle reference
      UPDATE settings
  ...
  IF settings_record.next_cycle_id IS NOT NULL THEN
    SELECT * INTO next_cycle
    FROM nomination_cycles
    WHERE id = settings_record.next_cycle_id;

    IF next_cycle IS NULL THEN
      -- Fix invalid next cycle reference
      UPDATE settings
  ...
  IF next_cycle.id IS NOT NULL AND next_cycle.start_date <= CURRENT_TIMESTAMP AND 
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Log error details
      RAISE NOTICE 'Error starting new cycle: %', SQLERRM;
      -- Rollback any partial changes
      RAISE EXCEPTION 'Failed to start new nomination cycle';
    END;
  END IF;
END;
$$ LANGUAGE plpgsql;