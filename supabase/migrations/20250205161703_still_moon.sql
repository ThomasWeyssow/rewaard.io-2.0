...
BEGIN
  ...
  IF NEW.next_nomination_start_date IS NOT NULL AND NEW.next_nomination_end_date IS NULL THEN
    ...
    -- Ensure next_nomination_period is set
    IF NEW.next_nomination_period IS NULL THEN
      NEW.next_nomination_period := 'monthly';
    END IF;
  ...
  IF TG_OP = 'UPDATE' AND 
    ...
    NEW.next_nomination_period := 'monthly'; -- Set default period
...
BEGIN
  ...
  IF settings_record.ongoing_nomination_end_date < CURRENT_TIMESTAMP THEN
    ...
    VALUES (
      ...
      COALESCE(settings_record.ongoing_nomination_period, 'monthly'),
  ...
  IF settings_record.next_nomination_start_date <= CURRENT_TIMESTAMP AND 
    ...
    SET
      ...
      ongoing_nomination_period = COALESCE(next_nomination_period, 'monthly'),
