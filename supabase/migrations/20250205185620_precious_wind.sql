-- First, clear cycle references from settings
UPDATE settings
SET 
  next_cycle_id = NULL,
  ongoing_cycle_id = NULL;

-- Then update any references to non-completed cycles in nominations
...
-- Update nomination_history references
...
BEGIN
  ...
  IF settings_record.ongoing_nomination_end_date < CURRENT_TIMESTAMP THEN
    ...
    IF settings_record.ongoing_nomination_start_date IS NOT NULL AND 
      ...
      -- Move nominations to history with the new cycle_id
