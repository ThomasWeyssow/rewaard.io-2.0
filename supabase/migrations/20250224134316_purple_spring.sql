...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND 
    ...
    -- Move next cycle to ongoing, preserving the period
    ...
    NEW.next_nomination_period := NULL;  -- Reset period for next cycle
    NEW.next_nomination_end_date := NULL;  -- Will be calculated when period is set
