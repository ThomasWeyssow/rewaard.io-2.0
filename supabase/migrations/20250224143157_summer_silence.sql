...
RETURNS TRIGGER AS $$
BEGIN
  -- Only calculate next_nomination_end_date if start date is set
  IF NEW.next_nomination_start_date IS NOT NULL THEN
    ...
    -- Generate new cycle_id if needed
    IF NEW.ongoing_cycle_id IS NULL THEN
    ...
    ELSIF NEW.next_nomination_period = 'bi-monthly' THEN
    ...
    RAISE NOTICE 'Calculated dates for next cycle:';
  ...
  IF TG_OP = 'UPDATE' AND 
    ...
    NEW.next_nomination_period := 'monthly';  -- Default to monthly for next cycle
    NEW.next_nomination_end_date := NULL;  -- Will be calculated when period is updated
    ...
    RAISE NOTICE '- Ongoing cycle ID: %', NEW.ongoing_cycle_id;
